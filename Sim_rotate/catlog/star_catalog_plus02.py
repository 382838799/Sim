# 优化后的第二版，读取.gz 修改为了读取.csv,且输出结果正确   2.9--- 40-----11min
import pandas as pd
import numpy as np
...
def filter_data_by_fov_and_mag(path, ra_cam, dec_cam, radius_deg, min_mag, max_mag, nside, output_dir, title=None):
    ...
    # 获取符合条件的像素块文件 - 修改为读取普通CSV文件
    try:
        pixel_files = [f for f in os.listdir(path) if f.startswith('pixel_') and f.endswith('.csv')]
    except Exception as e:
    ...
    for pixel_file in relevant_files:
        ...
        try:
            # 使用 chunksize 分批读取普通CSV文件以减少内存使用
            for chunk in pd.read_csv(pixel_file_path, usecols=cols, chunksize=100000):
                # 星等筛选
...
def process_from_csv(csv_file, input_dir, fits_output_dir, excel_output_dir, start_row=0, end_row=None, workers=None):
    ...
    # 设置进程数
    if workers is None:
        workers = max(1, cpu_count() // 4 + 2)  # 默认使用一半的CPU核心
...
if __name__ == '__main__':
    # 设置参数（恢复原来的参数设置方式）
    csv_file = '/mnt/d/Simulation/5.00000.csv'  # CSV 文件路径，即输入的相机指向
    input_dir = '/mnt/d/gaia/data_csv/'  # 768个星表文件所在的文件夹路径
    fits_output_dir = '/mnt/d/Simulation/output/chapter24/xydata_csv'  # FITS文件输出目录
    excel_output_dir = '/mnt/d/Simulation/output/chapter24/xydata_csv'  # Excel文件输出目录
