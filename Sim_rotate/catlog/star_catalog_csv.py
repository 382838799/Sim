...
def filter_data_by_fov_and_mag(path, ra_cam, dec_cam, radius_deg, min_mag, max_mag, nside, output_dir, title=None,
    ...
    try:
        # 根据是否使用压缩文件确定文件后缀
        file_suffix = '.csv.gz' if use_compressed else '.csv'
        pixel_files = [f for f in os.listdir(path) if f.startswith('pixel_') and f.endswith(file_suffix)]
    except Exception as e:
    ...
    for pixel_file in relevant_files:
        ...
        try:
            ...
            # 使用 chunksize 分批读取以减少内存使用
            for chunk in pd.read_csv(pixel_file_path, usecols=cols, chunksize=100000, **read_kwargs):
                # 星等筛选
                mask = (chunk['phot_g_mean_mag'] >= min_mag) & (chunk['phot_g_mean_mag'] <= max_mag)
                if not mask.any():
                    continue

                # 添加筛选后的数据
                for col in cols:
                    all_data[col].append(chunk.loc[mask, col].values)
                total_rows += mask.sum()
        except Exception as e:
    ...
    try:
        ...
        structured_data = np.core.records.fromarrays([all_data[col] for col in cols], dtype=dtype)
...
def process_single_row(row, data, f, W, H, pixel_scale, min_mag, max_mag, input_dir, fits_output_dir, excel_output_dir,
    ...
    try:
        ...
        fits_file = filter_data_by_fov_and_mag(input_dir, ra_cam, dec_cam, radius_deg, min_mag, max_mag, nside,
                                               fits_output_dir, title, use_compressed)
...
def process_from_csv(csv_file, input_dir, fits_output_dir=None, excel_output_dir=None, start_row=0, end_row=None,
                     workers=None, use_compressed=False):
    """处理CSV文件中的所有行"""
    # 设置默认输出目录
    if fits_output_dir is None:
        fits_output_dir = os.path.join(os.path.dirname(input_dir), "fits_output")
    if excel_output_dir is None:
        excel_output_dir = os.path.join(os.path.dirname(input_dir), "excel_output")
    ...
    # 设置进程数
    if workers is None:
        workers = max(1, cpu_count() // 2 - 2)  # 默认使用一半的CPU核心
...
if __name__ == '__main__':
    ...
    input_dir = '/mnt/d/gaia/data_csv/'  # 768个星表文件所在的文件夹路径
    fits_output_dir = '/mnt/d/Simulation/output/chapter24/xydata03'  # FITS文件输出目录
    excel_output_dir = '/mnt/d/Simulation/output/chapter24/xydata03'  # Excel文件输出目录
    use_compressed = False  # 设置为False以读取.csv文件而非.csv.gz文件
