# 优化后的第三版，读取.parquet 而非 .csv，保持其他逻辑不变,交换了x,y
import pandas as pd
import numpy as np
...
def to_camera_and_pixel_coords(ra, dec, f, W, H, ra_cam, dec_cam, pixel_scale):
    ...
    try:
        ...
        if np.any(mask):
            # 计算相机坐标
            x_cam = -f * y_proj[mask] / z_proj[mask]
            y_cam = -f * x_proj[mask] / z_proj[mask]
...
if __name__ == '__main__':
    # 设置参数（恢复原来的参数设置方式）
    csv_file = '/mnt/d/XT/AAA_Simulation/GY/chapter24/Point/chapter24.csv'  # 保持CSV格式的输入参数文件
    ...
    excel_output_dir = '/mnt/d/XT/AAA_Simulation/GY/chapter24/StarProperties/'  # Excel文件输出目录
